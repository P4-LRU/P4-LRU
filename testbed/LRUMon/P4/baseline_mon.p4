...
/**************************** PREPROCESSING ********************************/
	...
	action preprocessing_1() {
        meta.rand = random_hash.get({hdr.submit_record.src_addr,hdr.submit_record.dst_addr,hdr.submit_record.src_port,hdr.submit_record.dst_port,hdr.submit_record.protocol});
	}
...
/*********************** FP 1 **************************/
	...
	action insert_fp_action_1() {
		meta.key = insert_fp_salu_1.execute((bit<32>)hdr.lru_record.index[10:0]);
	}
...
/*********************** FP 2 **************************/
	...
	action insert_fp_action_2() {
		meta.key = insert_fp_salu_2.execute((bit<32>)hdr.lru_record.index[10:0]);
	}
...
/*********************** FP 3 **************************/
	...
	action insert_fp_action_3() {
		meta.key = insert_fp_salu_3.execute((bit<32>)hdr.lru_record.index[10:0]);
	}
